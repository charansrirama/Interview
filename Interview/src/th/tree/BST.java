...
public class BST {
	...
	if (x == null)
	    ...
	    if (x.leftChild.number > x.number)
		return false;
	    else
		return isBST(x.leftChild);
	} else if (x.rightChild != null && x.leftChild == null) {
	    if (x.rightChild.number < x.number)
		return false;
	    else
		return isBST(x.rightChild);
	} else {
	    if (x.leftChild.number > x.number || x.rightChild.number < x.number)
		return false;
	    else
		return isBST(x.leftChild) && isBST(x.rightChild);
	}
    }
	 ...
	 * 
	 * System.out.println(
	 * "How many numbers would you like to enter into the Red Black Tree?");
	 ...
	 * 
	 * System.out.println("\n\n****Display Start****"); for(int i=0; i<k;
	 * i++) { RBT.insert(inputarray[i]); }
	 * System.out.println(isBST(RBT.root)); RBT.display(RBT.getRoot());
