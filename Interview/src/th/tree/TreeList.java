...
class TreeList {
     ...
     */
    public static void join(Node2 a, Node2 b) {
	a.large = b;
	b.small = a;
    }
     ...
     */
    public static Node2 append(Node2 a, Node2 b) {
     ...
     */
    public static Node2 treeToList(Node2 root) {
	// base case: empty tree -> empty list
	if (root == null)
	    return (null);

	// Recursively do the subtrees (leap of faith!)
	Node2 aList = treeToList(root.small);
	Node2 bList = treeToList(root.large);
     ...
     */
    public static void treeInsert(Node2 root, int newData) {
	if (newData <= root.data) {
	    if (root.small != null)
		treeInsert(root.small, newData);
	    else
		root.small = new Node2(newData);
	} else {
	    if (root.large != null)
		treeInsert(root.large, newData);
	    else
		root.large = new Node2(newData);
	}
    }

    // Do an inorder traversal to print a tree
    // Does not print the ending "\n"
    public static void printTree(Node2 root) {
	if (root == null)
	    return;
